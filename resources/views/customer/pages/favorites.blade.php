...
<div style="position:relative;min-height:100vh;padding-top:88px;padding-bottom:60px;background:#131313;overflow:hidden;">
    ...
    <div style="position:relative;z-index:1;max-width:860px;margin:0 auto;padding:0 24px;">
        ...
        <div style="margin-bottom:28px;" class="fav-header-section">
            <h1 style="font-family:'Playfair Display',serif;font-size:2rem;font-weight:700;color:#fff;margin-bottom:4px;">Produk Favorit</h1>
            <p style="color:rgba(255,255,255,.4);font-size:.875rem;">Koleksi pilihan yang Anda simpan</p>
        </div>
        ...
        <div class="fav-list" style="display:flex;flex-direction:column;gap:12px;">
            ...
            <div class="fav-card-wrapper">
                <div class="fav-card glass-card" style="border-radius:16px;padding:20px;display:flex;align-items:center;gap:16px;">

                <img src="{{ $favorite->produk->image_url }}" alt="{{ $favorite->produk->nama_produk }}"
                     style="width:76px;height:76px;object-fit:cover;border-radius:12px;border:1px solid rgba(255,255,255,.08);flex-shrink:0;">

                <div style="flex:1;">
                    <p style="font-weight:600;font-size:1rem;color:rgba(255,255,255,.9);margin-bottom:4px;">{{ $favorite->produk->nama_produk }}</p>
                    <div style="display:flex;align-items:center;gap:8px;">
                        <span style="color:#c9a84c;font-weight:600;font-size:.9rem;">Rp {{ number_format($hargaDiskon, 0, ',', '.') }}</span>
                        @if($diskonPersen > 0)
                            <span style="background:rgba(201,168,76,.12);color:#c9a84c;font-size:9px;font-weight:700;padding:2px 8px;border-radius:999px;border:1px solid rgba(201,168,76,.2);">-{{ $diskonPersen }}%</span>
                        @endif
                    </div>
                </div>
                ...
                <div style="display:flex;align-items:center;gap:10px;flex-shrink:0;">
                    <a href="{{ route('customer.favorites.addToCart', $favorite->id) }}" class="btn-magnetic"
                       style="padding:9px 20px;background:linear-gradient(135deg,#c9a84c,#a8832d);color:#111;font-size:.82rem;font-weight:700;border-radius:999px;text-decoration:none;white-space:nowrap;transition:opacity .2s;"
                       onmouseenter="this.style.opacity='.85';"
                       onmouseleave="this.style.opacity='1';">+ Keranjang</a>

                    <form action="{{ route('customer.favorites.destroy', $favorite->id) }}" method="POST" style="display:inline;">
                        @csrf
                        @method('DELETE')
                        <button type="submit" onclick="return confirm('Hapus dari favorit?')" class="btn-magnetic"
                                style="width:36px;height:36px;background:rgba(239,68,68,.1);border:none;border-radius:8px;color:#ef4444;cursor:pointer;display:flex;align-items:center;justify-content:center;transition:background .2s;"
            ...
            <div class="glass-card" style="border-radius:20px;padding:64px;text-align:center;">
                ...
                <h3 style="font-family:'Playfair Display',serif;color:rgba(255,255,255,.8);font-size:1.4rem;margin-bottom:8px;">Belum Ada Favorit</h3>
                <p style="color:rgba(255,255,255,.4);margin-bottom:24px;">Tambahkan produk batik favorit Anda dari halaman koleksi</p>
                <a href="{{ route('customer.home') }}" class="btn-magnetic" style="display:inline-block;padding:12px 36px;background:linear-gradient(135deg,#c9a84c,#a8832d);color:#111;font-weight:700;border-radius:999px;text-decoration:none;font-size:.9rem;">Jelajahi Koleksi</a>
            </div>
            @endforelse
        </div>
        ...
        <div style="text-align:center;margin-top:24px;">
            <a href="{{ route('customer.home') }}" class="btn-magnetic"
               style="display:inline-block;padding:11px 32px;border:1.5px solid rgba(201,168,76,.4);color:#c9a84c;border-radius:999px;text-decoration:none;font-size:.875rem;font-weight:600;transition:all .25s;"
               onmouseenter="this.style.background='rgba(201,168,76,.1)';"
               onmouseleave="this.style.background='transparent';">❤️ Tambah Favorit Lain</a>
        </div>
        @endif
    </div>
</div>
